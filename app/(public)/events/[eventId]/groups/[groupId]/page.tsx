        ...
        <div className="grid gap-5">
          ...
          <Panel title="Members">
            <div className="mb-3 flex gap-2">
              <input className="h-10 min-w-0 flex-1 rounded-md border border-white/10 bg-slate-950 px-3 text-sm text-white outline-none focus:border-cyan-400" placeholder="User ID" value={memberUserId} onChange={(e) => setMemberUserId(e.target.value)} />
              <button className="primary-button" onClick={handleAddMember} type="button">Add</button>
            </div>
            {members.length ? (
              <div className="grid gap-2">
                {members.map((member) => (
                  <div className="flex items-center justify-between rounded-md border border-white/10 bg-slate-950 p-3 text-sm" key={member.id}>
