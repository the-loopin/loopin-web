...
function getDisplayCategory(event: EventItem) {
  return event.displayCategory?.trim() || formatCategoryName(event.category);
}
...
const createSteps = [
  { href: "#event-basic", title: "Basic Information", description: "Name, description and category" },
  { href: "#event-time", title: "Date & Time", description: "When your event happens" },
  { href: "#event-location", title: "Location", description: "Address and map marker" },
  { href: "#event-image", title: "Event Image", description: "Cover image and preview" },
  { href: "#event-details", title: "Additional Info", description: "Organizer and pricing" },
  { href: "#event-review", title: "Review", description: "Final publishing check" },
];
...
function getApiErrorMessage(caught: unknown, fallback: string) {
  ...
  return message.toLowerCase().includes("already exists") ? "This event already exists." : message;
}
...
export default function EventsPage() {
  ...
  function validateForm() {
    ...
    if (form.category === "OTHER" && !customCategory.trim()) nextErrors.customCategory = "Tell people what kind of event this is.";
    ...
    if (!form.isFree && (!Number.isFinite(Number(form.price)) || Number(form.price) <= 0)) {
      nextErrors.price = "Paid events need a valid price.";
    }
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      setActionToast({
        ...
        message: "Fix the first marked field, then publish again.",
      });
  ...
  function handleSaveDraft() {
    setActionToast({
      ...
      message: "Your current event details are still here while you keep editing.",
    });
  }
        ...
        <div className="create-workspace">
          <div className="create-workspace-header">
            <div>
              ...
              <h2>Create Event</h2>
              <p>Fill in the details below to host your event and connect with people nearby.</p>
            </div>
          ...
          <div className="create-workspace-grid">
            <aside className="create-step-panel" aria-label="Create event sections">
              ...
              <div className="create-side-tip">
                ...
                <strong>Tip</strong>
                <span>A clear title, accurate location, and strong cover image help people decide faster.</span>
              </div>
            </aside>

            <form className="create-event-form create-section-stack" id="create-event-form" onSubmit={handleCreate}>
              <section className="create-section-card" id="event-basic">
                ...
                <div className="create-form-grid">
                  <label className="form-field wide-field" data-create-field="title">
                    ...
                    {formErrors.title ? <em>{formErrors.title}</em> : <small>Make it specific enough to understand at a glance.</small>}
                  </label>
                  ...
                  {form.category === "OTHER" ? (
                    <label className="form-field" data-create-field="customCategory">
                      ...
                      {formErrors.customCategory ? <em>{formErrors.customCategory}</em> : <small>Tell people what kind of event this is.</small>}
                    </label>
                  ) : null}
                </div>
              </section>

              <section className="create-section-card" id="event-time">
                <div className="create-section-heading">
                  <span><Clock size={18} /></span>
                  <div>
                    <h3>Date & Time</h3>
                    <p>Choose a start and end time that gives people enough notice.</p>
                  </div>
                </div>
                <div className="create-form-grid">
              ...
              <section className="create-section-card" id="event-location">
                <div className="create-section-heading">
              ...
              <section className="create-section-card" id="event-image">
                <div className="create-section-heading">
              ...
              <section className="create-section-card" id="event-details">
                <div className="create-section-heading">
                  <span><Ticket size={18} /></span>
                  <div>
                    <h3>Additional Info</h3>
                    <p>Set organizer details and choose whether the event is free or paid.</p>
                  </div>
                </div>
                <div className="create-form-grid">
              ...
              <section className="create-section-card create-review-card" id="event-review">
                <div className="create-section-heading">
                  <span><CheckCircle2 size={18} /></span>
                  <div>
                    <h3>Review</h3>
                    <p>Make sure everything looks right before publishing.</p>
                  </div>
                </div>
                <div className="create-review-grid">
                  <span><strong>{form.isFree ? "Free event" : `${form.price || 0} AZN`}</strong><small>Price</small></span>
                  ...
                  >
                    <div className="event-card-inner">
                      <div className="event-card-face event-card-front">
                        <div
                          className={`event-poster poster-violet ${event.imageUrl ? "event-poster-image" : ""}`}
                          style={event.imageUrl ? { backgroundImage: `url(${event.imageUrl})` } : undefined}
                        >
                          {!event.imageUrl ? <CalendarDays size={20} /> : null}
                          <span>{getDisplayCategory(event)}</span>
                        </div>
