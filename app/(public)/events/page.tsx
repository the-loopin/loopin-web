      ...
      <div className="split-view-container">
              ...
              return (
                ...
                >
                  ...
                  <div className="event-card-body">
                    <div>
                      <p className="text-xs text-[var(--color-coral)] font-bold">{new Date(event.startDateTime).toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                      <h3 className="text-lg font-bold text-[var(--color-ink)] mt-0.5">{event.title}</h3>
                    </div>
                    <div className="flex flex-col gap-1 my-2">
                      <div className="event-meta-row"><MapPin size={13} className="text-[var(--muted)]" /> {event.address || event.city}</div>
                    ...
                    <div className="flex justify-between items-center border-t border-[var(--line)] pt-2 mt-1">
                      <span className="text-xs font-semibold text-[var(--color-ink)]">{event.isFree ? "Free" : `${event.price} AZN`}</span>
                      <Link className="text-xs text-[var(--color-accent)] font-bold hover:underline" href={`/events/${event.id}`}>
                        Open Event &rarr;
                      </Link>
                    </div>
                  </div>
        ...
        <div className="split-view-map">
          <div className="p-3 bg-[var(--color-surface)] border-b border-[var(--line)] flex justify-between items-center">
            <span className="text-xs text-[var(--muted)] uppercase font-bold tracking-wider">Baku Event Map</span>
            {selectedEventId && (
